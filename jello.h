...
// computes dotprodcut of three vectors, which are given as points 
// struct point vector1, vector2, dest
// resulte goes into dest
...
#define tCOMPUTE(rayORI, rayANGLE, a, b, c, d)\
\
  T = - (rayORI.x * a + rayORI.y * b + rayORI.z * c + d) / rayANGLE;

#define tINTERSECT(rayORI, rayDIR, T, dest)\
\
  (dest).x = rayORI.x + rayDIR.x * T;\
  (dest).y = rayORI.y + rayDIR.y * T;\
  (dest).z = rayORI.z + rayDIR.z * T;
